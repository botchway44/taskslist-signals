...
@Component({
  selector: 'ng-pass-otp',
  templateUrl: './ng-pass-otp.component.html',
  styleUrls: ['./ng-pass-otp.component.scss']
})
export class NgPassOtpComponent implements OnInit, AfterViewInit {
  @Input() config: Partial<Config> = { length: 4 };
  // tslint:disable-next-line: no-output-on-prefix
  @Output() onInputChange = new EventEmitter<string>();

  //@ts-ignore
  @Input() otpForm: any;
  @Output() otpFormChange = new EventEmitter<any>();
  ...
  rebuildValue() {
    ...
    this.otpFormChange.emit(this.otpForm)
  }

  getInputType(): string {
    return this.config.setTypePassword
      ? 'password'
